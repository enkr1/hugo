...
- **Future dates**: Hugo skips posts with `date` in the future — **including same-day future times**. This exact bug hid the parts-split retrospective on 2026-07-10 (post dated 21:30, pushed at 17:2x → deploy silently excluded it while everything else built fine; symptom: "post missing from live site, no errors anywhere"). Rule: frontmatter `date` must be ≤ `date '+%Y-%m-%dT%H:%M:%S%z'` **at commit time**, not just at drafting time. Never round up to a nice-looking time. The pre-commit hook now rejects future-dated content as a backstop.
